...
const Sidebar = ({ sideBarContent }: SideBarProps) => {
	...
	return (
		<Fragment>
			<div
				className={`transition-all h-[70px] w-full bottom-0 sm:top-0 sm:min-h-screen sm:h-full fixed lg:translate-x-0 sm:fixed z-[10000] sm:px-6 sm:pt-6 sm:pb-4 flex flex-col sm:w-[200px] bg-[#6272B9] sm:overflow-x-hidden ${toggleSidebar ? "sm:w-[200px]" : "sm:w-[64px]"
					}`}
			>
				...
				<button className="outline-none absolute flex-col sm:flex-row flex items-center top-4 sm:top-0 right-0 sm:relative">
					...
					/>
					<span
						className={`text-white text-xs sm:text-base pl-2 text-center pr-10 font-display ${!toggleSidebar ? "hidden" : "block"
							}`}
					>
						Logout
					</span>
